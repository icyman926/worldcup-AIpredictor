...
if (!source.includes("const GEMINI_MODEL = 'gemini-1.5-flash'")) {
  throw new Error('Gemini model is not pinned to gemini-1.5-flash.');
}
if (/gemini-2\.|gemini-2\.5|gemini-2\.0/.test(source)) {
  throw new Error('Forbidden Gemini 2.x model reference found.');
}
      ...
      text: async () => JSON.stringify({
        candidates: [{
          content: {
            parts: [{
              text: JSON.stringify({
                summary: 'Gemini 1.5 Flash mock context applied.',
...
sandbox.module.exports.handler(req, res).then(() => {
  ...
  if (!requestedUrl.includes('/models/gemini-1.5-flash:generateContent')) {
    throw new Error('Gemini request did not use gemini-1.5-flash endpoint: ' + requestedUrl);
  }
  ...
  if (payload.contexts[0].model !== 'gemini-1.5-flash') {
    throw new Error('Response model was not gemini-1.5-flash.');
  }
  console.log('PASS Gemini 1.5 Flash mock integration uses the correct endpoint and changes context.');
